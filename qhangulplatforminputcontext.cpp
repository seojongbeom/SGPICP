...
#include "qhangulplatforminputcontext.h"
...
HanjaTable* QHangulPlatformInputContext::hanjaTable = nullptr;
...
QHangulPlatformInputContext::QHangulPlatformInputContext(const QString &keyboardType) {
    ...
    m_hic = nullptr;
    for(i = 0; i < n; i++) {
        const char *s = hangul_keyboard_list_get_keyboard_id(i);
        if (keyboardType == s) {
    ...
    hangul_ic_connect_callback(m_hic, "transition", (void*)onTransition, nullptr);
    m_candidateList = nullptr;
...
void QHangulPlatformInputContext::reset() {
    if (m_candidateList != nullptr && m_candidateList->isVisible()) {
...
QLocale QHangulPlatformInputContext::locale() const
{
    return m_locale;
}
